...
export const ClassificationTable = memo(function ClassificationTable({
  ...
  return (
    <section id="classification-table" className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="grid grid-cols-1 divide-y divide-gray-200 dark:divide-gray-700">
        {items.map((item) => (
          <div id={`item-${item.label.toLowerCase().replace(/\s+/g, '-')}`} className="grid grid-cols-3" key={`${item.label}-${item.value}`}>
